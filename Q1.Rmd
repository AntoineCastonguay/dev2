...
C_RVector2 <- function(t, vars, parms){
  with(as.list(c(parms, vars)), {
    # Modèle consommateur ressoure
    dR <- alpha*R*(1-R/1000) - beta*R*C # dR/dt
    dC <- gamma*R*C - sigma*C  # dC/dt
...
C_RVector3 <- function(t, vars, parms){
  with(as.list(c(parms, vars)), {
    # Modèle consommateur ressoure
    dR <- alpha*R*(1-R/1650) - beta*R*C # dR/dt
    dC <- gamma*R*C - sigma*C  # dC/dt
...
#   try(dessinSol(parms = c(alpha=i,beta=i/2,gamma=i/3,sigma=i)))
# }
# 0.31 0.155 0.155 0.93

# best res
dessinSol(parms = c(alpha=0.31,beta=0.156,gamma=0.157,sigma=0.931))
dessinSol(parms = c(alpha=0.17,beta=0.0245,gamma=0.03,sigma=0.4),func = C_RVector2)
dessinSol(parms = c(alpha=0.17,beta=0.0243,gamma=0.0295,sigma=0.401),func = C_RVector3)
