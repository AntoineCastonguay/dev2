...
C_RVector2 <- function(t, vars, parms){
  with(as.list(c(parms, vars)), {
    # Modèle consommateur ressoure
    dR <- alpha*R*(1-R/40) - beta*R*C # dR/dt
    dC <- gamma*R*C - sigma*C  # dC/dt
...
dessinSol(parms = c(alpha=0.2,beta=0.02,gamma=0.04,sigma=0.42),func = C_RVector2)
dessinSol(parms = c(alpha=0.35,beta=0.12,gamma=0.12,sigma=0.45),func = C_RVector2)
